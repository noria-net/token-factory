...
func TestWasmdExport(t *testing.T) {
	db := dbm.NewMemDB()
	gapp := NewTokenAppWithCustomOptions(t, false, SetupOptions{
	...
	newGapp := NewTokenApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, wasm.EnableAllProposals, simtestutil.NewAppOptionsWithFlagHome(t.TempDir()), emptyWasmOpts)
